...
class TrainingArguments(HfTrainingArguments):
    max_position_embeddings: Optional[int] = field(
        default=None, metadata={"help": "The maximum position embedding per segment."}
    )
    summary_length: int = field(
        default=0,
        metadata={
            "help": "Number of summary tokens. 0 allocates no space for summary tokens."
        },
    )
    accumulate_summary: bool = field(
        default=False,
        metadata={
            "help": "If True, summary tokens of all past segments will be accumulated "
    ...
    training_substeps: Optional[int] = field(
        default=1,
        metadata={
            "help": "How often to detach gradients (1 substep=standard training)"
        },
    )
    randomize_substeps: Optional[bool] = field(
        default=False,
        metadata={
            "help": "apply  strategy to determine substep lengths in each substep"
        },
    )
    segments_per_substep: int = field(
        default=2,
        metadata={
            "help": "Number of substeps per segments when using --randomize_substep"
        },
    )
    segment_lengths: List[int] = field(
        default_factory=list,
        metadata={
            "help": "Max. number of tokens compressed per segment in a substep. Applies only when substeps are not randomized."
        },
    )
    segment_gradient_checkpointing: bool = field(
        default=False,
        metadata={
            "help": "If True, gradient checkpointing will be used after each segment."
        },
    )
...
class ModelArguments:
    ...
    lora: bool = field(
        default=False,
        metadata={"help": "Whether to use parameter efficient fine-tuning."},
    )
