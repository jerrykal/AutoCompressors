...
class SubstepTrainer(BaseTrainer):
    ...
    def compute_loss(
        ...
        for substep in range(self.args.training_substeps):
            ...
            if os.getenv("FA_EVAL", False):
                out = model(
                    **inputs,
                    segment_lengths=sum(self.args.segment_lengths),
                    use_cache=False,
                )
                softprompt = None
            else:
            ...
            if return_output_and_metrics:
                ...
                self.add_metrics(
                    metrics, log_p, labels, prefix=f"substep_{substep}-avg-"
                )
                ...
                for i in range(num_segments):
                    ...
                    metrics[f"substep_{substep}-seg{i}-numtokens"] = (
    ...
    def training_step(
        ...
        if self.args.training_substeps > 0:
            ...
            if self.log_count % self.args.gradient_accumulation_steps == 0:
                ...
                self.loss_log["total_substeps"] = (
                    self._nested_gather(self.substep_count).sum().item()
                )
                for i in range(self.args.training_substeps):
                    self.loss_log[f"substep_{i}"] = (
                        self._nested_gather(self.loss_log[f"substep_{i}"]).mean().item()
                    )
    ...
    def random_segment_lengths(self, input_ids, num_segments):
        ...
        if num_segments > 1:
            min_segment_length = max(
                math.ceil((input_ids.size(1) - max_positions) / (num_segments - 1)), 2
            )

            total_variable_length = (
                input_ids.size(1) - min_segment_length * num_segments
            )
            if num_segments - 1 > total_variable_length:
                raise ValueError(
                    "The specified number of segments_per_substep cannot cover the entire input sequence."
                )
            breakpoints = torch.multinomial(
                torch.ones(total_variable_length), num_segments - 1
            )
            segment_lengths = torch.diff(
                breakpoints.sort(-1).values,
    ...
    def segment_input(self, inputs, substep):
        ...
        slices = torch.linspace(
            0,
            inputs["input_ids"].shape[-1],
            steps=self.args.training_substeps + 1,
            device=inputs["input_ids"].device,
            dtype=torch.long,
        )
        input_slice = {
            k: v[:, slices[substep] : slices[substep + 1]] for k, v in inputs.items()
        }
        if self.args.randomize_substeps:
            segment_lengths = self.random_segment_lengths(
                input_slice["input_ids"], self.args.segments_per_substep
            )
        else:
