...
def read_args():
    ...
    parser.add_argument(
        "--num_plaintext_demonstrations", required=False, type=int, default=0
    )
...
def get_model_tokenizer_device_isac(args):
    ...
    if "autocompressor-llama" in args.model_path.lower():  # LLaMA-2 AC
        ...
        model = auto_compressor.LlamaAutoCompressorModel.from_pretrained(
            args.model_path
        )
        is_ac = True
...
class PromptGenerator(torch.utils.data.Dataset):
    def __init__(
        ...
        random.seed(
            10**6 * seed
            + 10**3 * sum(num_softprompt_demonstrations)
            + num_plaintext_demonstrations
        )
        ...
        if self.dataset["balanced_sampling"]:  # create balanced sample
            ...
            staggered_idxs = [
                idx for sublist in zipped_label_wise_idxs for idx in sublist
            ]
            num_total_demonstrations = (
                sum(self.num_softprompt_demonstrations) + num_plaintext_demonstrations
            )
            if len(staggered_idxs) < num_total_demonstrations:
                # repeat the list if there aren't enough examples
                staggered_idxs = staggered_idxs * (
                    num_total_demonstrations // len(staggered_idxs) + 1
                )
            start_splice_pos = random.randint(
                0, len(staggered_idxs) - num_total_demonstrations
            )
            sample_idxs = staggered_idxs[
                start_splice_pos : start_splice_pos + num_total_demonstrations
            ]
        else:  # using random sampling
            sample_idxs = random.sample(
                range(len(self.dataset["train"])),
                sum(num_softprompt_demonstrations) + num_plaintext_demonstrations,
            )
        ...
        if (
            sum(self.num_softprompt_demonstrations) > 0
        ):  # if softprompt demonstrations are needed
            ...
            softprompt_examples = iter(
                [
                    dict(zip(softprompt_examples, i))
                    for i in zip(*softprompt_examples.values())
                ]
            )  # unzip dict
            chunked_softprompt_examples = [
                list(itertools.islice(softprompt_examples, 0, i))
                for i in num_softprompt_demonstrations
            ]
            ...
            for chunk in chunked_softprompt_examples:
                (
                    softprompt_demonstrations_tokens,
                    chunked_softprompt_demonstration_count,
                ) = self.get_demonstrations_tokens(
                    chunk, add_special_tokens=add_special_tokens
                )
                chunked_softprompt_demonstrations_tokens.append(
                    softprompt_demonstrations_tokens
                )
                chunked_softprompt_demonstration_counts.append(
                    chunked_softprompt_demonstration_count
                )
                add_special_tokens = False
            self.all_softprompts_demonstrations_tokens = (
                chunked_softprompt_demonstrations_tokens  # list of torch.Tensor
            )
            self.num_softprompt_demonstrations = (
                chunked_softprompt_demonstration_counts  # revised list of int
            )

        if (
            self.num_plaintext_demonstrations > 0
        ):  # if plaintext demonstrations are needed
            plaintext_examples = self.dataset["train"][plaintext_idxs]
            plaintext_examples = [
                dict(zip(plaintext_examples, i))
                for i in zip(*plaintext_examples.values())
            ]  # unzip dict
            self.plaintext_demonstrations_tokens, self.num_plaintext_demonstrations = (
                self.get_demonstrations_tokens(
                    plaintext_examples,
                    add_special_tokens=(sum(self.num_softprompt_demonstrations) == 0),
                )
            )

    def get_demonstration_string(
        self, example: dict, label=None, include_label=True, for_calibration=False
    ) -> str:
        """
        ...
        example["label"] = (
            label if label is not None else example["label"]
        )  # override label
        example["answer"] = (
            example["options"][example["label"]] if include_label else ""
        )
        if for_calibration:
    ...
    def get_demonstrations_tokens(
        self, examples: list, add_special_tokens: bool, max_tokens=float("inf")
    ):
        """
        ...
        for example in examples:
            demonstration_string = (
                self.get_demonstration_string(example) + self.delimiter
            )
            extended_demonstrations_string = (
                demonstrations_string + demonstration_string
            )
            extended_demonstrations_tokens = self.tokenizer.encode(
                extended_demonstrations_string, add_special_tokens=add_special_tokens
            )
        ...
        demonstrations_tokens = self.tokenizer.encode(
            demonstrations_string,
    ...
    def get_calibration_nlls(
        ...
        is_ac: bool,
        softprompt=None,
        plaintext_demonstrations_tokens=None,
    ):
        """
        ...
        """
        assert (sum(self.num_softprompt_demonstrations) == 0) or (
            softprompt is not None
        )
        assert (self.num_plaintext_demonstrations == 0) or (
            plaintext_demonstrations_tokens is not None
        )
        add_special_tokens = (
            self.num_plaintext_demonstrations + sum(self.num_softprompt_demonstrations)
        ) == 0

        unanswered_example_string = self.get_demonstration_string(
            example, include_label=False, for_calibration=True
        )
        unanswered_example_tokens = self.tokenizer.encode(
            unanswered_example_string,
            add_special_tokens=add_special_tokens,
            return_tensors="pt",
        ).to(device)
        ...
        for label_idx in range(len(example["options"])):
            answered_example_string = self.get_demonstration_string(
                example, label=label_idx, for_calibration=True
            )
            answered_example_tokens = self.tokenizer.encode(
                answered_example_string,
                add_special_tokens=add_special_tokens,
                return_tensors="pt",
            ).to(device)
            option_tokens = answered_example_tokens[
                :, unanswered_example_tokens.shape[1] :
            ]
            option_length = option_tokens.shape[1]
            plaintext_tokens = (
                answered_example_tokens
                if plaintext_demonstrations_tokens is None
                else torch.cat(
                    [plaintext_demonstrations_tokens, answered_example_tokens], dim=1
                )
            )

            with torch.no_grad():
                calibration_option_logits = (
                    model.forward(
                        plaintext_tokens, softprompt=softprompt, use_cache=False
                    )["logits"][:, -option_length - 1 : -1, :]
                    if is_ac
                    else model.forward(plaintext_tokens, use_cache=False)["logits"][
                        :, -option_length - 1 : -1, :
                    ]
                )
                calibration_log_softmax = torch.log_softmax(
                    calibration_option_logits, dim=-1
                )
                calibration_nll = -torch.mean(
                    calibration_log_softmax.gather(
                        dim=2, index=option_tokens.unsqueeze(-1)
                    )
                )
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        unanswered_example_string = self.get_demonstration_string(
            test_example, include_label=False
        )
        unanswered_example_tokens = self.tokenizer.encode(
            unanswered_example_string,
            add_special_tokens=add_special_tokens,
...
def main(args):
    ...
    if use_softprompt:
        ...
        for (
            softprompt_demonstrations_tokens
        ) in prompt_generator.all_softprompts_demonstrations_tokens:
            assert (
                softprompt_demonstrations_tokens.shape[1] <= 2048
            ), "Softprompt too long!"
            with torch.no_grad():
    ...
    plaintext_demonstrations_tokens = (
        prompt_generator.plaintext_demonstrations_tokens.to(device)
        if use_plaintext_demonstrations
        else None
    )
    ...
    for example in progress_bar:
        ...
        for option_idx in range(len(example["answer_options"])):
            answered_example_tokens = example["answered_example_options"][
                option_idx
            ].to(device)
            ...
            with torch.no_grad():
                ...
                conditioned_log_softmax = torch.log_softmax(
                    conditioned_answer_logits, dim=-1
                )
                conditioned_nll = -torch.mean(
                    conditioned_log_softmax.gather(
                        dim=2, index=option_tokens.unsqueeze(-1)
                    )
                )
        ...
        conditioned_nlls = (
            torch.tensor(conditioned_nlls) - calibration_nlls
            if args.use_calibration
            else torch.tensor(conditioned_nlls)
        )
