...
class AutoCompressorMixin:
    ...
    def setup_autocompressor(self, config):
        ...
        ), "Compressor requires a summary_length config parameter"

        self.summary_config = SummaryConfig()

        if config.summary_length > 0:
            self.embed_summary = nn.Embedding(
                config.summary_length, self.get_input_embeddings().embedding_dim
            )
    ...
    def forward_segment(
        ...
        segment_last_hiddens = outputs.last_hidden_state[
...
class OPTLearnedPositionalEmbeddingWithPadding(nn.Embedding):
    ...
    def __init__(
        ...
        self.summary_config = (
            summary_config if summary_config is not None else SummaryConfig()
        )
    ...
    def forward(
        ...
        left_placeholder = torch.ones(
            bsz,
            self.summary_config.softprompt_length,
            dtype=torch.long,
            device=attention_mask.device,
        )  # <pad> -> zero vector
        right_placeholder = torch.ones(
            bsz,
            self.summary_config.summary_length,
            dtype=torch.long,
            device=attention_mask.device,
        )  # <pad> -> zero vector

        total_softprompt_length = (
            self.summary_config.softprompt_length
            + self.summary_config.past_key_values_softprompt_length
        )
        attention_mask = attention_mask[
            :,
            total_softprompt_length : attention_mask.size(1)
            - self.summary_config.summary_length,
        ]
